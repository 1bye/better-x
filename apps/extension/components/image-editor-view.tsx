// biome-ignore-all lint/performance/noJsxPropsBind: Canvas tools bind object-specific pointer actions and are not memoized.
...
const ImageEditorView = forwardRef<ImageEditorHandle, ImageEditorViewProps>(
  function ImageEditorComponent(
    ...
    const setEditorView = useCallback((next: ViewTransform): void => {
    ...
    const fitView = useCallback((): void => {
      ...
      const scale = clamp(
        Math.min(
          availableWidth / currentLayout.canvasWidth,
          availableHeight / currentLayout.canvasHeight
        ),
        0.05,
        4
      );
      setEditorView({
        scale,
        x: (bounds.width - currentLayout.canvasWidth * scale) / 2,
        y: (bounds.height - currentLayout.canvasHeight * scale) / 2,
      });
    }, [setEditorView]);
